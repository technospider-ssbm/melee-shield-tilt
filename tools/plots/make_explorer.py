...
function draw() {
  ...
  const shapes = [];
  // untilted body hurtboxes, neutral ink, always shown for reference
  for (const b of d.boxes[0][0]) {
    shapes.push({ type: 'path', path: stadiumPath(...b), fillcolor: 'rgba(82,81,78,0.35)',
      line: { color: 'rgba(82,81,78,0.5)', width: 0.5 }, layer: 'below' });
  }
  // posed hurtboxes at the current stick position, accent tint
  for (const b of (d.boxes[ai] ? d.boxes[ai][mi] : [])) {
    shapes.push({ type: 'path', path: stadiumPath(...b), fillcolor: 'rgba(42,120,214,0.25)',
      line: { color: 'rgba(42,120,214,0.6)', width: 0.6 }, layer: 'below' });
  }
