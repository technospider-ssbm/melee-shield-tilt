...
- **Yoshi (Ys): DONE, root cause found and fixed.** technospider traced it in
  the decomp: Yoshi's shield uses character-specific motion states
  (`ftYs_MS_GuardOn_0`=341, `ftYs_MS_GuardHold`=342, `ftYs_MS_GuardOff`=343;
  `ftYoshi/ftyoshi.c:29-51`, `ftyoshiguard.c`) and stores his state in
  `fp->mv.ys.guard`, a different union layout than the shared
  `fp->mv.co.guard` (fp+0x2344/+0x2348) every other character uses. That's
  why `guard_x4`/`guard_x8` read `0.0`/`1.05` — those bytes just aren't his
  tilt fields. And libmelee's shared `Action` enum has no name for raw
  motion_id 342, so it decodes as the nearby `NEUTRAL_B_CHARGING` — Yoshi
  **was** shielding the whole time; only the diagnostic was wrong.
  - **Fix** (`capture.py`): for `code == "Ys"`, accept a sample when the raw
    `motion_id` (direct RAM read, not libmelee's decoded action name) equals
    342, with an extra fixed 30-frame settle after reaching it (instead of
    the `Action.SHIELD` check used for everyone else). Kept the
    `facing_dir > 0` check.
  - **Result:** 49/49 reachable, **max/mean position error = 0.0000**
    against `data/Ys.csv` — every sample settles to the exact same position
    `(0, 5.88, 0)` regardless of stick input, confirming the brief's fixed-
    bubble prediction.
  - **Radius check:** live `axis_scale` (world-matrix column norms) is a
    constant **6.3** across all 49 samples (isotropic, `aniso=0.0000`),
    exactly matching `data/Ys.csv`'s `shield_radius_full` **and**
    `shield_radius_min` columns, both `6.2999997` — Yoshi's bubble doesn't
    scale with shield health or lightshield at all (matches
    `ftyoshiguard.c`'s `hurt.scale = 1;`, a hardcoded constant rather than
    the health/lightshield-dependent `inlineB0` formula other characters
    use). `initial_shield_size` for Yoshi is 6 in `characters.json`; the
    live/predicted 6.3 includes the parent chain's ~1.05 uniform scale on
    top of that.
